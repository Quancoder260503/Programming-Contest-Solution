...
struct MinCostFlow
{
    ...
    lint minCostFlow(int K, int src, int sink)
    {
        lint flow = 0;
        lint cost = 0;
        vector<lint> dist(N);
...
signed main(void)
{
    ...
    for (int i = 0; i < q; i++)
    {
        ...
        {
            if (at + curr.size() <= n + 1 && curr == s.substr(at, (int)curr.size()))
            {
                // cout << at << " " << at + curr.size() << '\n';
                mcf.addEdge(at, at + curr.size(), 1, -cost);
            }
        }
    }
    cin >> flow;
    cout << -mcf.minCostFlow(flow, src, sink) << '\n';
    return 0;
}