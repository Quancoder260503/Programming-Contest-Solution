...
void brute_force(vector<int>a) {
    int n = a.size(); 
    vector<int>cc(n); 
    iota(cc.begin(), cc.end(), 1); 
    map<int, int>ctr; 
    do
    {
        vector<int>b = a; 
        for(int i = 0; i < cc.size(); i++) { 
            for(int j = cc[i]; j <= n; j += cc[i]) { 
                b[j - 1] = 0; 
            }
            for(int i = 0; i < n; i++) if(b[i]) ctr[b[i]]++; 
        }
    }
    while(next_permutation(cc.begin(), cc.end()));
    for(const auto &[c, x] : ctr) cout << c << " " << x << " " '\n';  
    return; 
}
*/ 
int main(void) { 
    ...
    { 
        ...
        for(int i = 1; i <= n - k + 1; i++) { 
            res += P(n - k, i - 1) * mint(k) * (i - 1) * fact[n - i]; 
        }
        return res; 
    };
    mint ret = 0;
    map<int, mint>dyn; 
    for(int i = 0; i < n; i++) { 
        if(!dyn.count(ctr[i + 1])) { 
            dyn[ctr[i + 1]] = calc(n, ctr[i + 1]); 
        }
        ret += dyn[ctr[i + 1]] * mint(a[i]); 
    }
    cout << ret * ifact[n] << '\n'; 
    return 0; 
}