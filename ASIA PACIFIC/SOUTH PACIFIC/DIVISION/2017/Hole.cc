...
signed main(void)
{
    ...
    while (cin >> N >> W >> H)
    {
        ...
        for (int i = 0; i < N; i++)
        {
            Point p, q;
            cin >> p >> q;
            A.push_back(Line(p, q));
        }
        int ret = 1;
        for(int i = 0; i < N; i++)
        {
            if(segmentIntersect(from, to, A[i].A, A[i].B)) 
            {
                ret = 0; 
                break;  
            }
        ...
        A.push_back(Line(Point(0, 0), Point(W, 0)));
        A.push_back(Line(Point(0, 0), Point(0, H)));
        A.push_back(Line(Point(0, H), Point(W, H)));
        A.push_back(Line(Point(W, 0), Point(W, H)));
        auto get_side = [&](Line line, Point a)
        {
            int dir = ccw(line.A, line.B, a);
            if (dir == 0)
                return dir;
            return (dir < 0 ? -1 : 1);
        };
        auto reflection = [&](Line line, Point a)
        {
            Vector u = line.A - line.B;
            Vector v = line.A - line.B;
            double mult = ((u * (line.A - a)) / u.norm());
            v.x *= mult;
            v.y *= mult;
            v = (line.A - a) - v;
            v.x *= 2;
        ...
        auto chk = [&](Line a, Line f1, Line f2)
        {
            Point p1, p2;
            if(areParallel(f1, f2)) return true;
            areIntersect(a, f1, p1);
            areIntersect(a, f2, p2);
            if ((a.B - a.A).x * (p2.x - p1.x) >= 0 && (a.B - a.A).y * (p2.y - p1.y) >= 0) return true;
            return false;
        };
        for (int i = 0; i < A.size(); i++)
        {
            int side_from = get_side(A[i], from);
            int side_to = get_side(A[i], to);
            if (side_from == 0 || side_to == 0) continue;
            if (side_from != side_to) continue;
            int ctr = 0;
            for (int _ = 0; _ < 2; _++)
            {
                auto reflect = reflection(A[i], to);
                Line orien(from, reflect);
                int ret = !(cmp(orien.f(A[i].A), 0) == 0 || cmp(orien.f(A[i].B), 0) == 0); 
                for (int j = 0; j < A.size(); j++)
                {
                    if (i == j) continue; 
                    if (segmentIntersect(orien.A, orien.B, A[j].A, A[j].B) && chk(orien, A[j], A[i]))
                    {
                        ret = false;
                        break;
                    }
...
// The problem statement should include how will the ball bounce (or i did not read the statement properly)