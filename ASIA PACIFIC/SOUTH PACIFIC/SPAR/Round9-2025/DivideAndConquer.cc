...
int main() { 
 ...
 int n, m; 
 cin >> n; // n problems, m solvers 
 vector<double>h(n);
 for(auto &x : h) cin >> x; 
 cin >> m; 
 vector<double>d(m); 
 for(auto &x : d) cin >> x; 
 vector<vector<double>>s(n, vector<double>(m)); 
 for(int i = 0; i < n; i++) { 
  for(int j = 0; j < m; j++) { 
   cin >> s[i][j]; 
  } 
 }
 MCMF<ll, ll>min_cost_solver(n + m + 2); 
 int src = n + m, sink = src + 1; 
 for(int i = 0; i < n; i++) { 
   min_cost_solver.add(src, i, h[i], 0, 0);
 } 
 for(int i = 0; i < m; i++) { 
   min_cost_solver.add(i + n, sink, d[i], 0, 0);
 } 
 for(int i = 0; i < n; i++) { 
  for(int j = 0; j < m; j++) { 
   min_cost_solver.add(i, n + j, LLONG_MAX / 2, 0, ll(scale / s[i][j])); 
  } 
 } 
 cout << 1. * min_cost_solver.max_flow_min_cost(src, sink).second / scale << '\n'; 
 return 0; 
}