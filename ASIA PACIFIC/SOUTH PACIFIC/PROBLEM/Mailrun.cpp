...
signed main(void)
{
    ...
    int src = 0;
    int sink = 2 * n + 1;
    mcf.addEdge(src, s, 2, 0); 
    mcf.addEdge(s, s + n, 2, 0); 
    mcf.addEdge(t, t + n , 2, 0); 
    mcf.addEdge(t + n, sink, 2, 0); 
    for (int i = 1; i <= n; i++)
    {
        if(i == s || i == t) continue; 
        mcf.addEdge(i, n + i, 1, 0);
    }
    for (int i = 0; i < m; i++)
    {
        int u, v, w;
        cin >> u >> v >> w;
        mcf.addEdge(u + n, v, 1, w);
        mcf.addEdge(v + n, u, 1, w); 
    }
    pair<int, int> ans = mcf.minCostFlow(src, sink);
