...
int64_t resmn[maxf], resmx[maxf];
int N;

void get_max(vector<int64_t> &a, vector<int64_t> &b)
{
    sort(a.begin(), a.end());
    sort(b.begin(), b.end());
    int pa = lower_bound(a.begin(), a.end(), 0) - a.begin();
    int pb = lower_bound(b.begin(), b.end(), 0) - b.begin();
    int l = min(pa, pb);
    int r = max(pa, pb);
    vector<int64_t> ret;
    for (int i = 0; i < l; i++)
        ret.push_back(1LL * a[i] * b[i]);
    for (int i = r; i < N; i++)
        ret.push_back(1LL * a[i] * b[i]);
    int len = r - l;
    vector<uint32_t> fa(len), fb(len);
    for (int i = 0; i < len; i++)
    {
        fa[i] = (uint32_t)abs(a[l + i]);
        fb[i] = (uint32_t)abs(b[l + len - i - 1]);
    }
    ...
    vector<uint64_t> cret(2 * len + 1);
    int LOG = 1;
    while ((1 << LOG) < 2 * len + 1)
        LOG++;
    fft::multiprime_convolute_64(cret.data(), 2 * len + 1, fa.data(), len + 1, fb.data(), len + 1, LOG);
    for (int i = 0; i < len; i++)
    {
        resmx[ret.size() + i + 1] = acc - cret[i];
    }
    return;
}

void get_min(vector<int64_t> &a, vector<int64_t> &b)
{
    for (int i = 0; i < N; i++)
        a[i] = -a[i];
    sort(a.begin(), a.end());
    ...
    vector<uint64_t>cret(2 * len + 1);
    int LOG = 1;
    while((1 << LOG) < 2 * len + 1) LOG++;
    fft::multiprime_convolute_64(cret.data(), 2 * len + 1, fa.data(), len + 1, fb.data(), len + 1, LOG);
    for (int i = 0; i < len; i++)
    {
        resmn[ret.size() + i + 1] = acc + cret[i];
    }
    */
    return;
}
