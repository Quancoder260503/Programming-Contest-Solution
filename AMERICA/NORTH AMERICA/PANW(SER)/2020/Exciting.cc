...
signed main(void)
{
    ...
    while(cin >> n)
    {
        ...
        {
            cin >> power[i] >> games[i]; 
            mcf.addEdge(src, i, games[i]  - 1, 0);
            mcf.addEdge(i + n, sink, 1, 0);
            mcf2.addEdge(src, i, games[i] - 1, 0);
            mcf2.addEdge(i + n, sink, 1, 0);
            at = max(at, {power[i], i}); 
        }
        mcf.addEdge(src, at.second, 1, 0); 
        mcf2.addEdge(src, at.second, 1, 0); 
        for(int i = 0; i < n; i++)
        {
            for(int j = 0; j < n; j++)
            {
                if(power[i] > power[j])
                {
                    mcf.addEdge(i, n + j, 1, 1LL * power[i] xor power[j]);
                    mcf2.addEdge(i, n + j, 1, -1LL * (power[i] xor power[j]));
                }
            }
        }
        auto [flow, cost]  = mcf.minCostFlow(src, sink); 
        auto [flow2, cost2] = mcf2.minCostFlow(src, sink); 
        cout << cost << " " << -cost2 << '\n';
    }
    return 0; 
}