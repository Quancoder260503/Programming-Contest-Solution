...
int main(void)
{
    ...
    Matrix<double>matA(N, N + 1), matB(N, N + 1); 
    for(int i = 0; i < M; i++)
    {
        int U, V; 
        cin >> U >> V;
    ...
    for(int i = 0; i < N; i++)
    {
        ...
        if(arr[i].first == -1)
        {
            for(auto v : g[i]) 
            {
                matA[i][v] = -1.0  / (int) g[i].size(); 
                matB[i][v] = -1.0  / (int) g[i].size();  
            }
        }
        else 
        {
            matA[i][N] = arr[i].first; 
            matB[i][N] = arr[i].second; 
        }
    }
    auto retx = matA.gauss(); 
    auto rety = matB.gauss(); 
    cout << fixed << setprecision(10); 
    for(int i = 0; i < N; i++)
    {
        retx[i][N] /= retx[i][i]; 
        rety[i][N] /= rety[i][i];
        cout << retx[i][N] << " " << rety[i][N] << '\n'; 
    }
    return 0; 
}