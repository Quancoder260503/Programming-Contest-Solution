    ...
    Matrix operator*(const Matrix &r) const
    {
        ...
        for (int i = 0; i < n_row; i++)
        {
            for (int k = 0; k < n_col; k++)
            {
                ...
                {
                    res[i][j] = (res[i][j] + this->get(i, k) % mod * r.get(k, j) % mod + mod) % mod;
                }
            }
        }
        return res;
...
signed main(void)
{
    ...
    while (cin >> N >> b)
    {
        ...
        for (int i = 0; i < 90; i++)
        {
            for(int j = 0; j < 100; j++)
            {
                if(j % 10 == j / 10 || j / 10 == i % 10) continue; 
                if((i * 100 + j) % M  == b)
                {
                    res = (res + mod + ret[0][i]) % mod; 
                }
            }
        }
        cout << res << '\n'; 
