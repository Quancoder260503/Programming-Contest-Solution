...
{
    int N, M;
    vector<vector<int>>mat = {{4, 3, 2, 1}, {8, 7, 6, 5}, {12, 11, 10, 9}}; 
    while(cin >> N >> M && N && M)
    {
        ...
        for(int i = 0; i < M; i++)
        {
            ...
            int Y, C1, C2, C3, C4; 
            cin >> Y >> C1 >> C2 >> C3 >> C4;  
            mcf.addEdge(i, M + C1, 1, -mat[Y - 1][0]);
            mcf.addEdge(i, M + C2, 1, -mat[Y - 1][1]);
            mcf.addEdge(i, M + C3, 1, -mat[Y - 1][2]);
            mcf.addEdge(i, M + C4, 1, -mat[Y - 1][3]);
        }
