...
vector<BigInt>mul(vector<BigInt>a, vector<BigInt>b)
{
    vector<BigInt> ret(b.size() + a.size(), BigInt(0));
...
int main(void)
{
    ...
    for(int i = 0; i < dice.size(); i++)
    {
        ...
        cin >> x;
        ctr += x; 
        vector<BigInt>b(dice[i] + 1, 1);
        b[0] = 0; 
        for(int j = 0; j < x; j++) a = mul(a, b);
    }
    vector<int>ord; 
    for(int i = 0; i < a.size(); i++) if(a[i] > BigInt(0))
    {
        ord.push_back(i); 
    }
