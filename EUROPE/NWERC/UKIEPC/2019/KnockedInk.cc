...
int main(void) { 
    ...
    ld area; 
    cin >> n >> area;
    vector<ld>rx(n), ry(n), rr(n); 
    for(int i = n - 1; i >= 0; i--) { 
        ld x, y, r; 
        cin >> x >> y >> r; 
        rx[i] = cosl(angle) * x + sinl(angle) * y;
        ry[i] = cosl(angle) * y - sinl(angle) * x;
        rr[i] = r; 
    }
    ld left = 1e100; 
    for(int i = 0; i < n; i++) left = min(left, rr[i]); 
    ld right = left + sqrtl(area / acos(-1));
    for(int _ = 0; _ < 150; _++) {
        ld middle = (left + right) / 2.0;
        auto cc = rr; 
        for(int i = 0; i < n; i++) {
            cc[i] = max(0.0L, middle - rr[i]); 
        } 
        ld ret = total_area(rx, ry, cc); 
        if(ret < area) { 
            left = middle; 
        }
        else { 
