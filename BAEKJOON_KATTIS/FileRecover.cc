...
int main(void) {
  ...
  while(cin >> S) {
    if(S == "*") break; 
    suffix_array suffixArray(S.c_str()); 
    int64_t ret = 0; 
    int prev = 0; 
    for(int j = 0; j + 1 < sz(suffixArray.SA); j++) { 
      if(prev < suffixArray.lcp(j, j + 1)) { 
        ret += suffixArray.lcp(j, j + 1) - prev; 
      }
      prev = suffixArray.lcp(j, j + 1); 
    }
    cout << ret << '\n'; 
  }
  return 0; 
