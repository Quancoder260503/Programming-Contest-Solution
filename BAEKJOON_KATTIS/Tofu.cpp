...
// Pairity : even or odd of cells index
int n, m;
int table[5][5] = {{10, 8, 7, 5, 1}, {8, 6, 4, 3, 1}, {7, 4, 3, 2, 1}, {5, 3, 2, 2, 1}, {1, 1, 1, 1, 0}};
int dx[4] = {1, -1, 0, 0};
int dy[4] = {0, 0, -1, 1};
char g[MAXN][MAXN];
int to(int i, int j)
        ...
        for (int j = 1; j <= m; j++)
        {
            if ((i + j) & 1)
            {
                ...
                for (int k = 0; k < 4; k++)
                {
                    ...
                    if (inside(px, py))
                    {
                        // cout << i << " " << j << " " << px << " " << py << " " << to(i, j) << " " << to(px, py) << " " << table[g[i][j] - 'A'][g[px][py] - 'A']<<'\n';
                        mcf.addEdge(to(i, j), to(px, py), 1, -table[g[i][j] - 'A'][g[px][py] - 'A']);
                    }
                }
            }
