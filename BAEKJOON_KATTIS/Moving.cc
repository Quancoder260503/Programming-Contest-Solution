...
signed main(void)
{
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < n; j++)
        {
            mcf.addEdge(to(i, j), to(i, j) + N, 1, -g[i][j]);
            mcf.addEdge(to(i, j), to(i, j) + N, INF, 0);
            if (i + 1 < n)
            {
                mcf.addEdge(to(i, j) + N, to(i + 1, j), INF, 0);
            }
            if (j + 1 < n)
            {
                mcf.addEdge(to(i, j) + N, to(i, j + 1), INF, 0);
            }
        }
    }
    mcf.addEdge(fsrc, to(0, 0), INF, 0);
    mcf.addEdge(to(n - 1, n - 1) + N, sink, INF, 0);
