    ...
    Matrix operator*(const Matrix &r) const
    {
        ...
        for (int i = 0; i < n_row; i++)
        {
            for (int k = 0; k < n_col; k++)
            {
                ...
                {
                    res[i][j] = (res[i][j] + mod + 1LL * this->get(i, k) % mod * r.get(k, j) % mod) % mod;
                }
            }
        }
        return res;
...
signed main(void)
{
    ...
    while(T--)
    {
        cin >> N; 
        prob++; 
        Matrix<int64_t>coef(2, 2);
        Matrix<int64_t>ret(2, 1); 
        coef[0][0] = 6;  coef[0][1] = -4; 
        coef[1][0] = 1;  coef[1][1] = 0; 
        ret[0][0]  = 6;   ret[1][0]  = 2; 
        ret = coef.pow(N - 1) * ret; 
        ret[0][0]--; 
        string S = to_string(ret[0][0]); 
