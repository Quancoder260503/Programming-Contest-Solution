...
const lint inf = 1e9;
// Template code by Nguyen Thanh Trung RR
...
void solve()
{
    ...
    for (auto [sx, sy] : A)
    {
      //  cout << src << " " << to(sx, sy) << endl;
        mcf.addEdge(src, to(sx, sy), 1, 0);
    }
    for (auto [ex, ey] : B)
    {
      //  cout << to(ex, ey) << " " << sink << endl;
        mcf.addEdge(to(ex, ey), sink, 1, 0);
    }
    for (auto [sx, sy] : A)
    {
        for (auto [ex, ey] : B)
        {
            mcf.addEdge(to(sx, sy), to(ex, ey), 1, abs(sx - ex) + abs(sy - ey));
        }
    }
