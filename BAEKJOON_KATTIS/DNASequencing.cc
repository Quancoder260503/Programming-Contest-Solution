...
int main(void) {
  ...
  while(cin >> S >> T) {
    ...
    first++;  
    int c = 127; 
    string combine = S + char(c) + T;
    suffix_array suffixArray(combine.c_str()); 
    vector<int>SA = suffixArray.SA; 
    int ret = 0; 
    for(int i = 0; i + 1 < sz(combine); i++) { 
      if((SA[i] < sz(S) && SA[i + 1] > sz(S)) || (SA[i] > sz(S) && SA[i + 1] < sz(S))) { 
        ret = max(ret, suffixArray.lcp(i, i + 1)); 
      }
    }
    if(ret == 0) { 
      ...
      for(int i = 0; i + 1 < sz(combine); i++) { 
        if((SA[i] < sz(S) && SA[i + 1] > sz(S)) || (SA[i] > sz(S) && SA[i + 1] < sz(S))) { 
          if(suffixArray.lcp(i, i + 1) == ret) { 
            ans.insert(combine.substr(SA[i]).substr(0, ret)); 
          }
        }
      }
